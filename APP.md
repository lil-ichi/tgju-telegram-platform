...
- 2026-09-19: Supercharged the Telegram AI Assistant (`tgju_engine_ai_assistant.py`) for ultra-low TTFT (Time To First Token), token streaming, and natural Persian financial grounding:
  ...
  3. Immediate Telegram Feedback & Typing Heartbeat: Telegram polling dispatches `sendChatAction(action="typing")` (<50ms) immediately, accompanied by a background thread heartbeat refreshing the typing state every 3s until completion so users perceive zero dead time.
  ...
  6. Provider Circuit Breaker & Fail-Fast: Added `_PROVIDER_FAILURES` in `tgju_engine_ai.py` with cooldown invalidation to skip unresponsive or dead endpoints in 0ms without waiting for repeat HTTP timeouts. Fast-failed HTTP 4xx/5xx errors without redundant non-streaming retries.
  7. Zero Thinking Latency & Lean Generation: Added `thinking: {"type": "disabled"}` and `chat_template_kwargs: {"thinking": False}` to prevent models from generating lengthy English scratchpad monologues; regex-stripped `<think>` tags in `_sanitize_identity`; capped assistant max tokens to 300-400 for instant responses; added reasoning fallback extraction in `_extract_chat_content`.
  8. Natural Iranian Market Persona: Upgraded `DEFAULT_SYSTEM_PROMPT` to a warm, authentic Persian market advisor persona that avoids robotic greetings, boilerplate, and robotic AI phrases.
  9. Dashboard Real-Time Simulator: Upgraded `#panel_assistant` in `tgju_platform_ui.html` with real-time SSE streaming, live TTFT badge (`⚡ اولین توکن (TTFT): ... ms`), and performance indicator card. All 107 tests pass.
