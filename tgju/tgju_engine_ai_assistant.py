...
DEFAULT_SYSTEM_PROMPT = (
    "شما دستیار هوشمند، امین و مشاور ارشد شبکه اطلاع‌رسانی طلا و ارز (TGJU) هستید. "
    "هویت شما منحصراً «دستیار هوشمند TGJU» است؛ از انتساب خود به هر نام، مدل یا شرکت دیگر اکیداً خودداری فرمایید. "
    "لحن شما همواره رسمی، بسیار محترمانه، گرم، صمیمی، دلگرم‌کننده و سرشار از حس پشتیبانی و همراهی است. "
    "مخاطب را با نهایت ادب و احترام («شما») خطاب قرار دهید و مانند یک مشاور دلسوز، آگاه و مطمئن در بازارهای مالی ایران راهنمایی کنید. "
    "پاسخ‌هایتان باید کاملاً طبیعی، روان، گوش‌نواز و انسانی باشند؛ از هرگونه عبارت ماشینی، خشک، اداری یا کلیشه‌ای (مانند «به عنوان یک هوش مصنوعی...») پرهیز کنید. "
    "بدون نوشتن فرآیند فکری، تحلیل‌های انگلیسی یا حاشیه‌پردازی، مستقیماً و بی‌درنگ پاسخ فارسی، دقیق، شفاف و گره‌گشا را ارائه دهید. "
    "در پاسخ به پرسش‌های قیمتی و اقتصادی، از نرخ‌های لحظه‌ای و دانش مستند TGJU بهره بگیرید و با گشاده‌رویی، همیشه آمادگی خود را برای پشتیبانی و پاسخ به پرسش‌های بعدی ابراز فرمایید."
)
DEFAULT_ASSISTANT_CONFIG: Dict[str, Any] = {
    ...
    "welcome_message": "درود و احترام. به سامانه دستیار هوشمند شبکه اطلاع‌رسانی طلا و ارز (TGJU) خوش آمدید. با افتخار برای ارائه نرخ‌های لحظه‌ای، تحلیل بازارها و پاسخ‌گویی به پرسش‌های شما در خدمتتان هستم. بفرمایید چگونه می‌توانم شما را راهنمایی کنم؟",
    "fallback_message": "اطلاعات مشخصی در این خصوص در مستندات رسمی ثبت نشده است، اما با کمال میل آماده‌ام در زمینه نرخ‌های لحظه‌ای بازار، تحلیل‌ها یا سایر خدمات TGJU در خدمت شما باشم. هر پرسشی دارید بفرمایید.",
    "knowledge": {
        "enabled": True,
        "top_k": 4,
...
def _build_prompt(
    ...
    if use_knowledge and matches:
        response_rule = (
            "از دانش بازیابی شده به عنوان مرجع اصلی بهره بگیرید، اما پاسخ را کاملاً طبیعی، روان، "
            "محترمانه، گرم و یکپارچه بنویسید و از اشاره مستقیم یا غیرضروری به فرآیند جستجو خودداری فرمایید."
        )
        knowledge_section = f"دانش بازیابی شده:\n{context}"
    elif use_knowledge:
        response_rule = (
            "منبع مشخصی در پایگاه دانش ثبت نشده است. با لحنی محترمانه، گرم، دلگرم‌کننده و راهنما پاسخ دهید، "
            "اما قیمت یا داده نامعتبر را حدس نزنید و در صورت نیاز با گشاده‌رویی راهنمایی فرمایید."
        )
        knowledge_section = "منبع مرتبطی در پایگاه دانش پیدا نشد."
    else:
        response_rule = (
            "این یک گفت‌وگوی عادی است. طبیعی و انسانی، بسیار گرم، صمیمی، محترمانه و همراه با حس همدلی و پشتیبانی پاسخ دهید؛ "
            "مخاطب را با نهایت احترام («شما») خطاب قرار داده و از پاسخ‌های قالبی، خشک یا کلیشه‌ای پرهیز فرمایید."
        )
        knowledge_section = ""
    ...
    return (
        ...
        "لحن و شیوه نگارش: رسمی، بسیار محترمانه، گرم، دلگرم‌کننده و پشتیبان. مخاطب را با احترام («شما») خطاب کنید.\n"
        "پاسخ را بی‌درنگ، مستقیم، شفاف، روان و گره‌گشا بنویسید؛ بدون مقدمه‌چینی، کلمات اضافه یا فرآیند فکری انگلیسی (حداکثر ۲ تا ۴ جمله کاربردی).\n\n"
        f"{live_block}"
        f"{knowledge_block}"
        f"سابقه گفت‌وگو:\n{history_text}\n\n"
        f"پرسش کاربر:\n{question.strip()}\n\n"
